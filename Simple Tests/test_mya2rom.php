...
<body>
  <header>
  <h1>Mya2Rom.js Test Cases (Auto)</h1>
  <p>Compares auto-romanisations with the data inside MOW</p>
  <p>NOTE: Might not always be validated as correct, even if it's correct, because the romanisation data in MOW can be modified or have alternatives provided and thus not match up.</p>
  <p><b>Testing only IPA for now, since only IPA data is in MOW</b></p>
  </header>
...
for (var i = 0; i < mya_words.length; i++){
  if(mya_words[i].getAttribute("lang") == "my"){
    ...
    var auto_ipa = mya2rom(mya_word, "ipa", false).trim();
